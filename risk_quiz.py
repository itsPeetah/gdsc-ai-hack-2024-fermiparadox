def get_risk_tolerance_coefficient():
    ...
    return calculate_risk_tolerance(answers)

def calculate_risk_tolerance(answers):
    weights = {
    ...
    for question, response in answers.items():
        ...
        if question in weights and response in weights[question]:
            total_score += weights[question][response]
            max_score += max(weights[question].values()) 

    if max_score > 0:
        risk_tolerance_coefficient = (total_score / max_score)
    else:
        risk_tolerance_coefficient = 0 
    return risk_tolerance_coefficient

def provide_recommendations(rtc):
    if rtc < 0.34:
...
def main():
    risk_tolerance_coefficient = get_risk_tolerance_coefficient()
    print("User Risk Tolerance Coefficient:", risk_tolerance_coefficient)
    provide_recommendations(risk_tolerance_coefficient)
